...
def getAnnotationsAbstract(paper_id, text):
    ...
    for i in range(len(annotations)):
        ...
        if annotations[i]['rho'] > 0.5 and annotations[i]['link_probability'] > 0.5:
            ann = ann.capitalize()
...
def getAnnotationsFieldStudy(paper_id, text):
    url = url_endpoint + token + '&text=' + text
    ...
    for i in range(len(annotations)):
        ann = annotations[i]['spot']
        if annotations[i]['rho'] > 0.03 and annotations[i]['link_probability'] > 0.03:
            ann = ann.capitalize()
            ann = dbr + ann.replace(' ', '_')
            # Guardar como recursos de la Dbpedia:
            for c in annotations[i]['dbpedia_categories']:
